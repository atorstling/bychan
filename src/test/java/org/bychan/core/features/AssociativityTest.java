...
public class AssociativityTest {
    ...
    public void right() {
        Language<Integer> l = new LanguageBuilder<Integer>().named("pow")
                ...
                .newToken().named("pow").matchesString("^").infixParseAs((previous, match, parser, currentPowerFloor) -> (int) Math.pow(previous, parser.subExpression(previous, currentPowerFloor - 1)))
                .completeLanguage();
