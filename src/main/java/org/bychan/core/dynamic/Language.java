...
public class Language<N> {
    ...
    @NotNull
    private final Map<TokenKey, DynamicTokenType<N>> tokenTypesByKey;
    ...
    public Language(@NotNull final String name, @NotNull final List<TokenDefinition<N>> tokenDefinitions) {
        ...
        // and DynamicTokenFinder. First build all token types with an empty finder, then build the
        ...
        tokenTypesByKey = dynamicTokenTypes.stream().collect(Collectors.toMap(DynamicTokenType::getKey, Function.identity()));
    ...
    @NotNull
    private DynamicTokenType<N> findTokenByKey(@NotNull TokenKey soughtKey) {
    ...
    private Collection<DynamicTokenType<N>> toTokenTypes(@NotNull final List<TokenDefinition<N>> leveledDefinitions, @NotNull final DynamicTokenFinder<N> tokenFinder) {
