...
public class PrioritiesTest {
    ...
    public void prioritiesAreObeyed() {
        Language<Integer> l = new LanguageBuilder<Integer>().named("parantheses")
                .newToken().named("mult").leftBindingPower(1).matchesString("*").led((previous, match, parser, lbp) -> previous * parser.subExpression()).end()
                .newToken().named("plus").leftBindingPower(2).matchesString("+").led((previous, match, parser, lbp) -> previous + parser.subExpression()).end()
