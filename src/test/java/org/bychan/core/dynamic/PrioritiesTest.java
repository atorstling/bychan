...
public class PrioritiesTest {
    ...
    public void prioritiesAreObeyed() {
        ...
        lb.newToken().named("mult").leftBindingPower(1).matchesString("*").led((left, parser, lexeme) -> left * parser.parseExpression(left, lexeme.leftBindingPower())).build();
        lb.newToken().named("plus").leftBindingPower(2).matchesString("+").led((left, parser, lexeme) -> left + parser.parseExpression(left, lexeme.leftBindingPower())).build();
